...
public class DayListAdapter extends RecyclerView.Adapter<DayListAdapter.ViewHolder> {
    ...
    public DayListAdapter(Context context, List<DayBean> data, OnItemClickListener listener) {
        mContext = context;
        mData = data;
        mListener = listener;
    }
    ...
    @Override
    public void onBindViewHolder(@NonNull @org.jetbrains.annotations.NotNull DayListAdapter.ViewHolder holder, int position) {
        if(holder == null) {
            return;
        }
        if(mData != null) {
